...
Figure~\ref{fig:gap} shows the outcome. The interpretability gap is
\emph{at most zero}: with $\mathcal{O}(10^5)$-jet background samples
the $S{+}D$ tree now \emph{exceeds} the measured transformer rejection
outright for $1 \leq \ctau \leq 30$~mm (e.g.\ $5.9\times 10^3$ versus
$1.2\times 10^3$ against $b$ jets at 1~mm, and test-sample-saturated
$> 8\times 10^4$ versus a measured $2.2\times 10^3$ at 3~mm), and the
single dEEC observable alone captures $65$--$80\%$ of the transformer's
log-rejection. The nominal-basis tree is the supervised counterpart of
the anomaly-detection blind spot: supervised training does extract a
lifetime-\emph{independent} $\sim 50\times$ $b$-jet rejection from
shape information alone, but the curve never rises with $\ctau$ ---
displacement is where all the lifetime discrimination lives, and the
analytic family transmits essentially all of it. The appropriate
caveat has sharpened accordingly: a compact transformer trained on
$\mathcal{O}(10^4)$ signal jets evidently does not realize the true
information ceiling, so Fig.~\ref{fig:gap} bounds the gap from the
side that matters --- the engineered observable basis loses nothing
relative to a constituent-level learner trained under identical
conditions, and a larger transformer could at best close back up to
the tree. \todo{seed ensembles for error bands; larger transformer at
