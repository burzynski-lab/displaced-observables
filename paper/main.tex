...
(standard basis, at every lifetime) to $49\%$ at $\ctau = 1$~mm and
$\geq 86\%$ for $\ctau \geq 10$~mm, with the QCD background model
...
\begin{enumerate}
  ...
  coincidence requirement further and is the strongest single
  observable at short lifetime, while the displacement-normalized
...
Figure~\ref{fig:vae} shows the result at truth level, for a
fully-connected (V)AE trained on inclusive QCD jets only
(60/20/20 train/validation/test split; anomaly thresholds set on the
QCD test sample). The blind spot is total: in basis $S$ the anomaly
...
lifts the efficiency at the $1\%$ working point to $49\%$ at
$\ctau = 1$~mm, $71\%$ at 3~mm, $86\%$ at 10~mm, and $\geq 95\%$ for
$\ctau \geq 30$~mm (VAE; the deterministic AE is comparable, slightly
weaker at short lifetime). The background model is stable under the
augmentation: the $b$-jet anomaly rate at the $1\%$ inclusive working
point rises only from $\sim 0.5\%$ to $\sim 1.5\%$, the anticipated
mild heavy-flavor tail that furnishes an in-situ validation region.
...
$51\%$ of the supervised efficiency at $\ctau = 1$~mm, $72\%$ at 3~mm,
$87\%$ at 10~mm, and $\geq 96\%$ for $\ctau \geq 30$~mm --- at long
...
per-flavor efficiency curves; latent-space and reconstruction
diagnostics; cross-check with the reco-level set-based architectures
(deep-set / DETR) of the companion framework.}

% =====================================================================
\section{The interpretability gap}
\label{sec:gap}

To bound what the observable family might be leaving on the table, we
train a fully supervised constituent-level classifier per $\ctau$
point: a permutation-invariant transformer (two encoder layers, four
heads, masked mean pooling) over the jet's tracks with per-track inputs
$(z_i, \Delta\eta_i, \Delta\phi_i,\allowbreak
\mathrm{sgn}(d_{0,i})\ln(1+|d_{0,i}|/\sigma_i), q_i)$ --- the identical
information content available to the observables. It is compared, on
the same jet splits, against a gradient-boosted tree on the $S{+}D$
observable basis, a tree on the nominal basis $S$, and the single best
observable $\mathrm{dEEC}(\min)$.

Figure~\ref{fig:gap} shows the outcome. At the present sample sizes the
interpretability gap is \emph{consistent with zero}: the $S{+}D$ tree
matches or exceeds the transformer at every lifetime, both saturating
the test-sample rejection ceiling for $\ctau \gtrsim 3$--$10$~mm, and
the single dEEC observable alone captures $70$--$88\%$ of the ceiling's
log-rejection for $\ctau \geq 1$~mm. The nominal-basis tree is the
supervised counterpart of the anomaly-detection blind spot: supervised
training does extract a lifetime-\emph{independent} $\sim 50\times$
$b$-jet rejection from shape information alone, but the curve never
rises with $\ctau$ --- displacement is where all the lifetime
discrimination lives, and the analytic family transmits essentially all
of it. Two caveats qualify the null gap: rejections at long lifetime
are capped by test statistics (open saturation in
Fig.~\ref{fig:gap}), and a compact transformer trained on
$\mathcal{O}(10^4)$ jets need not realize the true information ceiling.
Both are addressed by the high-statistics production; at development
scale the defensible statement is that the observable basis loses
nothing relative to a constituent-level learner trained under identical
conditions. \todo{high-statistics rerun; seed ensembles for error
bands}
...
\begin{figure}[tb]
  ...
  $\ctau(\pid)$ for the supervised constituent transformer, the
  gradient-boosted trees on bases $S{+}D$ and $S$, and the single best
  observable. Plateaus at $8.2\times 10^3$ reflect the test-sample
  statistics. Right: fraction of the transformer's log-rejection
  captured by each interpretable proxy. Truth-level tracking.}
  \label{fig:gap}
\end{figure}
