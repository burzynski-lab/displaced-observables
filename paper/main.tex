...
acceptance ceiling $r < r_{\max}$. Two scenarios span the range of realistic
tracking performance: \emph{standard} ($r_{1/2} = 40$~mm, $r_{\max} = 120$~mm) and
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.98\textwidth]{vae_recon_VAE_SpD_truth_displaced.pdf}
  ...
  reconstructed faithfully, while signal reconstructions are pulled onto the
...
Figure~\ref{fig:scenarios} compares the benchmark observables across the
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.95\textwidth]{scenario_comparison_paper.pdf}
  \caption{$b$-jet rejection at 50\% signal efficiency versus
  $\ctau(\pid)$ for the weighted multiplicity (left) and the dEEC
  (right) under the three tracking scenarios: truth (black), standard
  tracking (red), and standard + large-radius tracking (blue). Open
  triangles are sample-statistics lower bounds.}
  \label{fig:scenarios}
\end{figure}

\paragraph{Pileup.} Figure~\ref{fig:pileup} shows the benchmark
...
of that residual is genuinely displaced (strange-hadron daughters), it
is angularly uncorrelated and too rare
...
\begin{figure}[tb]
  \centering
  \includegraphics[width=0.95\textwidth]{pileup_paper_standard_lrt.pdf}
  \caption{$b$-jet rejection versus $\ctau(\pid)$ for the weighted
  multiplicity (left) and the dEEC (right), with ($\mu = 60$, red) and
  without ($\mu = 0$, black) minimum-bias pileup overlay, standard+LRT
  tracking. Open triangles are sample-statistics lower bounds.}
  \label{fig:pileup}
\end{figure}
...
Sec.~\ref{sec:adtrain}, while the displaced features are shown in
