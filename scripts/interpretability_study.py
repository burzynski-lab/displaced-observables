...
def main() -> None:
    ...
    methods = ["BDT S+D (ceiling)", "BDT S", "dEEC(min)", "$\\Sigma_i w_i$"]
    ...
    for ctau in ctaus:
        ...
        for mname, basis in (("BDT S+D (ceiling)", BASIS_S + BASIS_D), ("BDT S", BASIS_S)):
            bdt = XGBClassifier(
    ...
    styles = {"BDT S+D (ceiling)": ("k", "-", "o"), "BDT S": ("tab:gray", ":", "v"),
    ...
    for m in methods[1:]:
        frac = [np.log(max(results[(m, c, bname)], 1.001))
                / np.log(max(results[("BDT S+D (ceiling)", c, bname)], 1.002))
                for c in ctaus]
        col, ls, mk = styles[m]
